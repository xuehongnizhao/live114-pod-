...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        _imageView.image = _photo.placeholder; // 占位图片 宽100 高100
...
- (void)adjustFrame
{
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _imageView.frame = CGRectMake(rect.size.width/2 - 50, rect.size.height/2-50, 100, 100);
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)tap {
    ...
    NSLog(@"self.minscale:%f",self.minimumZoomScale);
    NSLog(@"self.maxScale:%f",self.maximumZoomScale);
