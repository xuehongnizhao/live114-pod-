			...
			if (idx == len) {
				...
				if (c == (unichar)'\n' || c == 0x0085) { // U+0085 is the NEXT_LINE unicode character
					...
					if (lastWrapCache.index > indexCache.index && lineBreakMode != NSLineBreakByCharWrapping &&
						(!lastLine || lineBreakMode != NSLineBreakByCharWrapping)) {
...
- (CGSize)drawInRect:(CGRect)rect withZFont:(ZFont *)font {
	return [self drawInRect:rect withZFont:font lineBreakMode:NSLineBreakByCharWrapping];
...
- (CGSize)sizeConstrainedToSize:(CGSize)size {
	return [self sizeConstrainedToSize:size lineBreakMode:NSLineBreakByClipping];
}
